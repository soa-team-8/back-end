...
namespace Explorer.API.Controllers.Author.Administration
{
    ...
    public class EncounterController : BaseApiController
    {
        ...
        public async Task<ActionResult<EncounterDto>> Create([FromForm] EncounterDto encounter,[FromQuery] long checkpointId, [FromQuery] bool isSecretPrerequisite, [FromForm] List<IFormFile>? imageF = null)
        {
            ...
            var response = await Client.PostAsync($"http://encounters_database:3030/encounters/author", formData);;
            var jsonResponse = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
                var encId = (long)JObject.Parse(jsonResponse)["id"];
                response = await Client.PutAsync($"http://tours_database:3000/checkpoints/setEnc/{checkpointId}/{encId}/{isSecretPrerequisite}", null);
            }
        ...
        public async Task<ActionResult<EncounterDto>> Update([FromForm] EncounterDto encounter, [FromForm] List<IFormFile>? imageF = null)
        {
            ...
            using HttpResponseMessage response = await Client.PutAsync("http://encounters_database:3030/encounters/" + encounter.Id, jsonContent);
            var jsonResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ActionResult> Delete(int id)
        {
            ...
            using HttpResponseMessage response = await Client.DeleteAsync("http://encounters_database:3030/encounters/" + id);
        ...
        public async Task<ActionResult<EncounterDto>> GetById(int id)
        {
            ...
            using HttpResponseMessage response = await Client.GetAsync("http://encounters_database:3030/encounters/" + id);
            var jsonResponse = await response.Content.ReadAsStringAsync();
